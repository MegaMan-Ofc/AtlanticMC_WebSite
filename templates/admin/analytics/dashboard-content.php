...
$periodLabel = (string) ($adminAnalytics['period_label'] ?? admin_analytics_period_label($analyticsDays));
...
$maxDailyRevenue = admin_analytics_max_metric($dailySales, 'revenue_cents');
$maxDailyOrders = admin_analytics_max_metric($dailySales, 'paid_orders');
$topSelling = $adminAnalytics['top_selling_products'] ?? [];
$topDisplayed = $adminAnalytics['top_displayed_products'] ?? [];
$topInteracted = $adminAnalytics['top_interacted_products'] ?? [];
...
$maxSold = admin_analytics_max_metric($topSelling, 'sold_quantity');
$maxDisplayed = admin_analytics_max_metric($topDisplayed, 'impressions');
$maxInteracted = admin_analytics_max_metric($topInteracted, 'interactions');
$maxCart = admin_analytics_max_metric($topCart, 'cart_additions');
$maxCategoryRevenue = admin_analytics_max_metric($categoryRows, 'revenue_cents');
$maxPageViews = admin_analytics_max_metric($topPages, 'page_views');
...
<div class="admin-analytics-toolbar">
    <div>
        <span><?= e(t('admin.analytics_period')) ?></span>
        <strong><?= e($periodLabel) ?></strong>
    </div>
    <div class="admin-analytics-periods" role="group" aria-label="<?= e(t('admin.analytics_period')) ?>">
        <?php foreach ([7, 30, 90, 36500] as $periodDays): ?>
            <button
                class="button button--ghost admin-analytics-period<?= $analyticsDays === $periodDays ? ' is-active' : '' ?>"
                type="button"
                data-admin-analytics-period="<?= $periodDays ?>"
                aria-pressed="<?= $analyticsDays === $periodDays ? 'true' : 'false' ?>"
            ><?= e($periodDays >= 36500 ? t('admin.analytics_period_all') : t('admin.analytics_period_days', ['days' => $periodDays])) ?></button>
        <?php endforeach; ?>
    </div>
</div>
...
<section
    ...
    aria-live="polite"
>
    <?php $adminTrafficExpanded = false; require TEMPLATE_PATH . '/admin/analytics/traffic-widget.php'; ?>
</section>
...
<section class="admin-analytics-section" aria-labelledby="admin-sales-title">
    <header class="admin-analytics-section-heading">
        <div>
            ...
            <p><?= e(t('admin.analytics_sales_text_period', ['period' => $periodLabel])) ?></p>
        </div>
        ...
        <article class="admin-panel admin-analytics-card admin-funnel-card">
            ...
            <div class="admin-funnel">
                <div class="admin-funnel-step admin-funnel-step--views">
                    <span><?= e(t('admin.analytics_product_sessions')) ?></span>
                    <strong><?= (int) ($funnel['product_sessions'] ?? 0) ?></strong>
                </div>
                <div class="admin-funnel-rate"><?= e(number_format((float) ($funnel['view_to_cart_rate'] ?? 0), 1, ',', '.')) ?>%</div>
                <div class="admin-funnel-step admin-funnel-step--cart">
                    <span><?= e(t('admin.analytics_cart_sessions')) ?></span>
                    <strong><?= (int) ($funnel['cart_sessions'] ?? 0) ?></strong>
                </div>
                <div class="admin-funnel-rate"><?= e(number_format((float) ($funnel['cart_to_sale_rate'] ?? 0), 1, ',', '.')) ?>%</div>
                <div class="admin-funnel-step admin-funnel-step--sales">
                    <span><?= e(t('admin.analytics_paid_orders')) ?></span>
                    <strong><?= (int) ($funnel['paid_orders'] ?? 0) ?></strong>
                </div>
            </div>
            <p class="admin-analytics-hint"><?= e(t('admin.analytics_funnel_hint_sessions')) ?></p>
        </article>
    </div>
</section>
...
<section class="admin-analytics-section" aria-labelledby="admin-products-intelligence-title">
    ...
    <div class="admin-analytics-grid admin-analytics-grid--four">
            ...
            <div class="admin-ranked-list">
                ...
                <?php foreach ($topSelling as $index => $row): ?>
                    <div class="admin-ranked-row">
                        ...
                        <div><strong><?= e((string) $row['name']) ?></strong><small><?= e(format_money((int) $row['revenue_cents'])) ?></small><span class="admin-rank-track"><i style="--rank-width: <?= max(4, (int) round(((int) $row['sold_quantity'] / $maxSold) * 100)) ?>%"></i></span></div>
        ...
        <article class="admin-panel admin-analytics-card">
            <div class="admin-card-heading"><div><span class="admin-card-kicker"><i class="fa-regular fa-eye" aria-hidden="true"></i><?= e(t('admin.analytics_exposure')) ?></span><h3><?= e(t('admin.analytics_most_displayed')) ?></h3></div></div>
            <div class="admin-ranked-list">
                <?php if ($topDisplayed === []): ?><p class="admin-analytics-empty"><?= e(t('admin.analytics_no_display_data')) ?></p><?php endif; ?>
                <?php foreach ($topDisplayed as $index => $row): ?>
                    <div class="admin-ranked-row">
                        <span class="admin-rank-number"><?= $index + 1 ?></span>
                        <div><strong><?= e((string) $row['name']) ?></strong><small><?= e(t('admin.analytics_unique_viewers_value', ['count' => (int) $row['unique_sessions']])) ?></small><span class="admin-rank-track"><i style="--rank-width: <?= max(4, (int) round(((int) $row['impressions'] / $maxDisplayed) * 100)) ?>%"></i></span></div>
        ...
        <article class="admin-panel admin-analytics-card">
            <div class="admin-card-heading"><div><span class="admin-card-kicker"><i class="fa-solid fa-arrow-pointer" aria-hidden="true"></i><?= e(t('admin.analytics_interest')) ?></span><h3><?= e(t('admin.analytics_most_interacted')) ?></h3></div></div>
            <div class="admin-ranked-list">
                <?php if ($topInteracted === []): ?><p class="admin-analytics-empty"><?= e(t('admin.analytics_no_interaction_data')) ?></p><?php endif; ?>
                <?php foreach ($topInteracted as $index => $row): ?>
                    <div class="admin-ranked-row">
                        <span class="admin-rank-number"><?= $index + 1 ?></span>
                        <div><strong><?= e((string) $row['name']) ?></strong><small><?= e(t('admin.analytics_interaction_sessions_value', ['count' => (int) $row['interaction_sessions']])) ?></small><span class="admin-rank-track"><i style="--rank-width: <?= max(4, (int) round(((int) $row['interactions'] / $maxInteracted) * 100)) ?>%"></i></span></div>
                        <b><?= (int) $row['interactions'] ?></b>
                    </div>
                <?php endforeach; ?>
            </div>
        </article>
